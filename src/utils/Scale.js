...
function Scaleplate(c){
	...
	if(outerArguments[1] != undefined && typeof(outerArguments[1]) == 'function'){
       outerArguments[1](c.def,c.percent);
    }
    ...
    if(canvas.getContext){    //简单地检测当前浏览器是否支持Canvas对象，以免在一些不支持html5的浏览器中提示语法错误
        ...
        for(var i=0; i<Math.ceil(mid/capacity); i++){
        	ctx.beginPath();
            if((i*capacity+c.start)%(space) == 0){
                ctx.moveTo(i*c.unit+space,Math.ceil(c.height*3/4));
            }else if((i*capacity+c.start)%(space/2) == 0){
        ...
        var x,now,moveDistance,cx,upx,upnow,nowx;
        ...
        canvas.addEventListener('touchmove',function(e){
            ...
            if((now<c.start) || now>c.end-1){  //判断临界值情况
                ...
                if (now>(c.end-1)) {
                        now = c.end-1; 
                        defX = Math.floor(c.width/2)-((c.end-c.start)/capacity*c.unit+space);
				}
			}
            drawImg(img,Math.floor(c.width/2)-((now-c.start)*c.unit/capacity+space),0);
            if(outerArguments[1] != undefined && typeof(outerArguments[1]) == 'function'){
            	outerArguments[1](now,c.percent);
            }
        },false);
        ...
        canvas.addEventListener('touchend',function(e){
            e.stopPropagation();
            e.preventDefault();
            if(now%(capacity) < capacity/2){
                while(now%(capacity)){
                    now--;
                }
            }else if(now%(capacity)>=capacity/2){
                while(now%(capacity)){
                    now++;
                }
            }
